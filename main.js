...
window.onload = function() {
	...
	file = file.split('/');
	if(file.indexOf('additem.html') != -1) {
		...
		el('featuredbox').innerHTML = "<img src='images/featured.png' alt='' /><a href='#' id='featurelink'></a>"
		var text = "";
		for (i=0; i<categories.length; i++) {
			text += "<a class='catlink' href='javascript:displayCategory(" + i + ")'>" + makeTitleCase(categories[i]) + "</a>" + "<br />";
		}
		el('sortcategories').innerHTML = text;
	}
};
...
// GLOBAL VARIABLE
var bedding = [ "sleeping bag", "sheets", "blankets", "pillow", "air mattress", "sleeping pad", "cot", "tarp", "air pump", "repair kit for air mattress","utility bags for storage" ];
...
var displayItem = function(item) {
	var fh3 = el('featuredh3');
	if (fh3.innerHTML == "Featured:")
		fh3.innerHTML = "Items:";
	var text = "";
	text += "<div class='listitem'>" + 																	// MATH - (multi line statement)
			para("Category", item['category']) + 
			para("Name", item['name']) + 
			para("Quantity", item['quantity']) + 
			para("Purchase By", item['date']) +
			para("Important", item['important']) +
			para("Comments", item['comments']) + 
			"<img src='images/" + item['category'] + ".png' />" +
			"</div>";
	el('featuredbox').innerHTML += text;
};

var displayCategory = function(cats) {
	var itemsArray = JSON.parse(jsonObject);
	var len = itemsArray.length;
	
	el('featuredbox').innerHTML = "";
	for (i=len-1; i>=0; i--) {
		if (itemsArray[i]['category'] == categories[cats]) {
			displayItem(itemsArray[i]);
		}
	}
};
